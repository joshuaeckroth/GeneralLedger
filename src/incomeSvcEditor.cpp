...
IncomeSvcEditor::IncomeSvcEditor(QWidget *parent, const char *name)
...
IncomeSvcEditor::~IncomeSvcEditor()
{
...
void IncomeSvcEditor::listSection(ElementSection *section)
{
    QPtrList<EditorElement> list;
    if(section == &income)
        list = db->getElements(Database::incomeSvcIncome);
    if(section == &expenses)
        list = db->getElements(Database::incomeSvcExpenses);
...
void IncomeSvcEditor::deleteSection(ElementSection *section)
{
...
void IncomeSvcEditor::updateSection(void *newSection)
{
...
void IncomeSvcEditor::addElement(int category)
{
    dialog.main = new QDialog;
    switch((Database::editorCategories)category)
    {
        case Database::incomeSvcIncome:
            dialog.main->setCaption("Add to Income");
            break;
        case Database::incomeSvcExpenses:
...
void IncomeSvcEditor::oneAccountToggled()
{
...
void IncomeSvcEditor::rangeAccountToggled()
{
...
void IncomeSvcEditor::addIncome()
{
    addElement((int)Database::incomeSvcIncome);
    updateSection(&income);
}

void IncomeSvcEditor::addExpenses()
{
    addElement((int)Database::incomeSvcExpenses);
    updateSection(&expenses);
}

void IncomeSvcEditor::updateAccounts()
{
    accounts = db->getAccountsList();
}
