...
PDFOptions::PDFOptions() : QObject(0)
{
    settings = Settings::instance();
}

QString PDFOptions::exec()
{
    ...
    if(question.dialog->exec())
    {
        ...
        options.paperSizeCustomWidthLineEdit->setFixedWidth(
                (options.paperSizeCustomWidthLineEdit->minimumSizeHint()).width() * 2);
        if(PDFPaperSize == 3)
        ...
        options.paperSizeCustomHeightLineEdit->setFixedWidth(
                (options.paperSizeCustomHeightLineEdit->minimumSizeHint()).width() * 2);
        if(PDFPaperSize == 3)
...
void PDFOptions::paperSizeChanged(int index)
{
    if(index != 3)
    {
        options.paperSizeCustomWidthLineEdit->setEnabled(false);
...
void PDFOptions::passwordNoClicked()
{
    options.passwordYesRadioButton->setChecked(false);
    options.passwordNoRadioButton->setChecked(true);
}

void PDFOptions::passwordYesClicked()
{
